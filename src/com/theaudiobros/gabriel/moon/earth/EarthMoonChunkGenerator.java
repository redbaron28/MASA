...
import com.theaudiobros.gabriel.mars.BedrockPopulator;
...
public class EarthMoonChunkGenerator extends ChunkGenerator {
    ...
    public ChunkData generateChunkData(World world, Random rand, int ChunkX, int ChunkZ, BiomeGrid biome) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                int multitude = 6; //how much we multiply the value between -1 and 1. It will determine how "steep" the hills will be.
                int sea_level = 64;

                double maxHeight = gen1.noise(realX, realZ, frequency, amplitude) * multitude + sea_level;
                for (int y = 0; y < maxHeight; y++) {
                    data.setBlock(x, y, z, Material.CONCRETE.getId(), (byte) 8);
                }
            }
        }
    ...
    public List<BlockPopulator> getDefaultPopulators(World world) {
        ...
        pops.add(new com.theaudiobros.gabriel.moon.earth.OrePopulator(MASA.getInstance()));
        return pops;
    }
